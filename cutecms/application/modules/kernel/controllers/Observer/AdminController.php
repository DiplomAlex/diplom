...
class Observer_AdminController extends App_Event_Observer
{
    ...
    public function initAction()
    {
        ...
        /* set current site */
        $this->_initMultisite($controller);
    ...
    protected function _initMultisite(Zend_Controller_Action $controller)
    {
        $controller->view->currentSiteId = $controller->getHelper('AdminMultisite')->getSiteId();
        $controller->view->isAllowedSiteSelect = $controller->getHelper('AdminMultisite')->isAllowedMultisite();        
    }
    ...
    {
        $view->layout()->adminBox_SiteSelect = $view->adminBox_SiteSelect();
    }
    ...
    protected function _initLocale(Zend_Controller_Action $controller)
    {
        if ( ! Zend_Registry::isRegistered('Zend_Translate')) {             
            ...
            $langService->setCurrent($langService->getDefault());
            $code2 = $langService->getDefault()->code2;
     ...
     protected function _initAcl(Zend_Controller_Action $controller, $throwExceptionInsteadOfRedirect)
     {
        ...
        if ( ! $allowed) {
            if ($throwExceptionInsteadOfRedirect === TRUE) {
                ...
                $controller->getHelper('Redirector')->gotoUrlAndExit($controller->view->stdUrl(NULL, 'login', 'admin-auth', 'kernel'));
            }
        }
     }
