	...
	for ($i = 0; $i < $len; $i++)
	{
		...
		if ($mState == 0)
		{
			...
			else
			{
				...
				trigger_error('App_Utf8::to_unicode: Illegal sequence identifier in UTF-8 at byte '.$i, E_USER_WARNING);
				return FALSE;
			}
		}
		else
		{
			...
			if (0x80 == (0xC0 & $in))
			{
				...
				if (0 == --$mState)
				{
					...
					if (((2 == $mBytes) AND ($mUcs4 < 0x0080)) OR
						...
						($mUcs4 > 0x10FFFF))
					{
						trigger_error('App_Utf8::to_unicode: Illegal sequence or codepoint in UTF-8 at byte '.$i, E_USER_WARNING);
						return FALSE;
					}
			...
			else
			{
				...
				trigger_error('App_Utf8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte '.$i, E_USER_WARNING);
				return FALSE;
			}
		}
	}
