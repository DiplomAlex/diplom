...
class App_Resource_Swf_Driver
{
    ...
    */
    function __construct($file="")
    {
        ...
        $head = App_Resource_Swf_File::read($this->file, 3);
        ...
        */
        ...
        App_Resource_Swf_File::rewind($this->file, "rb");
        if($head == "CWS"){
            $data = App_Resource_Swf_File::read($this->file, 8);
            @$_data = gzuncompress(App_Resource_Swf_File::read($this->file, filesize($this->file)));
            $data = $data . $_data;
            $this->data = $data;
            $this->compression = 1;
            $this->isValid = 1;
        } else if ($head == "FWS"){
            $this->data = App_Resource_Swf_File::read($this->file, filesize($this->file));
            $this->isValid = 1;
        } else {
            /**
        ...
        App_Resource_Swf_File::close($this->file, "rb");
    }
    ...
    */
    function write($filename, $overwrite = 1)
    {
        ...
        if(is_writeable(dirname($filename))){
            ...
            App_Resource_Swf_File::write ($filename, $newdata, $mode = "wb");
            App_Resource_Swf_File::close($filename, "wb");
            return true;
        } else {
            return false;
        }
