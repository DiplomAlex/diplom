...
(function ($) {
	...
	$.modal.impl = {
		...
		create: function (data) {
			...
			this.opts.borderId = 'simplemodal_borderer';			
			this.opts.borderCss = $.extend(this.opts.borderCss, {
					/* display: 'none',*/
					position: 'fixed', 
					zIndex: this.opts.zIndex + 2,
					height: this.opts.sizes[0],
					width: this.opts.sizes[1],
					top: this.opts.position[0],
					left: this.opts.position[1]
				});

			this.dialog.border = $('<div/>')
				.attr('id', this.opts.borderId)
				.addClass('simplemodal-border')
				.css(this.opts.borderCss)
				.appendTo(this.opts.appendTo);

		
			this.opts.position[0] += this.opts.borderSize;
			this.opts.position[1] += this.opts.borderSize;

			var containerSizes = [];

			containerSizes[0] = this.opts.sizes[0] - (2*this.opts.borderSize);
			containerSizes[1] = this.opts.sizes[1] - (2*this.opts.borderSize);
			...
			this.dialog.container = $('<div/>')
				...
				.append(this.opts.close && this.opts.closeHTML
					...
					: '')
				.appendTo(this.dialog.border);			

				
			this.dialog.wrap = $('<div/>')
				.attr('tabIndex', -1)
				.addClass('simplemodal-wrap')
				.css({height: containerSizes[0] - (2 * this.opts.wrapperPadding), outline: 0, padding: this.opts.wrapperPadding})
		...
		close: function () {
			...
			if ($.isFunction(this.opts.onClose) && !this.occb) {
				...
				this.dialog.border.hide().remove();
